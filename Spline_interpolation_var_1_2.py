...
"""
def func_plot_var_2_1_linear(func, f_i, n, a, b) -> list:
    ans = []
    x = np.linspace(a, b, num=n)
    for j in range(n):
        sum = 0
        for i in range(n):
            sum += (func(j - i)) * f_i(x[i])
        ans.append(sum)
    return ans
"""
...
def var2_1() -> None:
    ...
    for funct_plot, f_i, f_nodes in functions_plot:
        ...
        # if (n == 1):
        #     f1_plot_linear = []
        #     x_f1_plot_linear = np.linspace(ab[0], ab[1], num=n + 1)
        #     x = np.linspace(ab[0], ab[1], num=n + 1)
        #     for j in range(n + 1):
        #         sum = 0
        #         for i in range(n + 1):
        ...
        #     for k in range(n):
        #         f1_plot_linear = []
        #         x_f1_plot_linear = np.linspace(ab[k], ab[k + 1], num=n)
        #         x = np.linspace(ab[k], ab[k + 1], num=n)
        #         for j in range(n):
        #             sum = 0
        #             for i in range(n):
        #                 sum += (funct_plot(j - i)) * f_i(x[i])
...
"""
def func_plot_var_2_2(func, f_i, n, _a, _b, k):
    ans = []

    a1 = derivative(f_i, _a)
    b1 = derivative(f_i, _b)

    h = (_b - _a) / n

    x = np.linspace(_a, _b, num=n + 1)

    a = np.array([-1 / 2] + [0] + [1 / 2] + [0 for x in range(n)], dtype=float)

    void_matrix = np.empty((0, n + 2), dtype=float)

    b_matrix = []
    for i in range(n + 1):
        b = np.array([0 for x in range(i)] + [1 / 6] + [2 / 3] + [1 / 6] + [0 for x in range(n - i)], dtype=float)
        b_matrix.append(b)

    c = np.array([0 for x in range(n)] + [-1 / 2] + [0] + [1 / 2], dtype=float)
    ...
    d = np.array([h * a1] + [f_i(x[i]) for i in range(n + 1)] + [h * b1], dtype=float)

    A = void_matrix

    roots = np.linalg.solve(A, d)
    x = np.linspace(_a, _b, num=n + 3)
    for i in range(n + 3):
        ans.append((func(x[i] - k)) * roots[i] * 1.49)
    return ans
"""

def var_2_2() -> None:
    functions_plot = [(f_cubic, f_arbitrary_1, f_f_arbitrary_nodes_1), (f_cubic, f_arbitrary_2, f_f_arbitrary_nodes_2)]
    for funct_plot, f_i, f_nodes in functions_plot:
        n = int(input("Enter n:"))
        _n = 1000
        a, b = tuple(map(float, input("Enter a and b: ").split(' ')))
        ...
        # y_new = f(x_new)
        #
        # plt.plot(x_new, y_new, label='SciPy Cubic', color='yellow')
